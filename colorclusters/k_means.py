...
def _choose_initial_centroids(k_value, datapoints):
    if not datapoints:
        return []
    centroids = [datapoints[0]]*k_value
    return centroids
...
class KMeans():
    ...
    def compute_until_predicate(self, predicate):
        while not predicate(self):
            self.shift_centroids()

    def compute_until_max_distance(self, max_distance):
        self.compute_until_predicate(lambda self: max(self.shift_distance) <= max_distance)
