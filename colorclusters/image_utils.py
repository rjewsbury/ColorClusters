...
def map_index_to_paletted_image(img, index_data, colors):
...
def map_to_paletted_image(img, colors, distance=euclidean):
    ...
    """
    index_data = map_pixels_to_closest_color_index(img.getdata(),colors,distance)

    return map_index_to_paletted_image(img, index_data, colors)
